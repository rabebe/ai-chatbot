...
class RAGSummarizer:
    ...
    def _get_llm(self, model_type: str, temperature: float, max_tokens: int):
        ...
        if model_type == "google":
            ...
            raise ValueError(
                f"Unsupported LLM model: {model_type}. Only 'google' is supported."
            )
    ...
    def _create_summarization_chain(self):
        ...
        chain = (
            {
                "context": self.chromadb_manager.get_retriever() | format_docs,
...
def create_rag_summarizer(
    ...
    return RAGSummarizer(chromadb_manager=chromadb_manager, llm_model=llm_model)
